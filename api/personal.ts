...
export interface PersonalTabCounts { all: number; mine: number; shared: number; favorite: number }

/**
 * 个人中心文档列表（tab: all/mine/shared/favorite），附带各 Tab 计数
...
export function apiGetPersonalDocs(params: Omit<PersonalListQuery, 'tab'> & { tab: Exclude<PersonalListQuery['tab'], 'handover'> }) {
	return useAuthFetch<ApiResult<PaginatedData<PersonalDocItem> & { tabCounts: PersonalTabCounts }>>('/api/personal/documents', {
		method: 'GET',
		query: params,
	})
}
...
export function apiGetPersonalHandover(params: { keyword?: string; page?: number; pageSize?: number }) {
	return useAuthFetch<ApiResult<PaginatedData<HandoverGroup> & { tabCounts: PersonalTabCounts }>>('/api/personal/documents', {
		method: 'GET',
