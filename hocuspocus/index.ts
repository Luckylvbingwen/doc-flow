 ...
 */
async function canEditDocument(userId: number, documentId: number): Promise<boolean> {
	const db = getPool()
 ...
 * 格式: doc-{id}
 */
function parseDocumentId(documentName: string): number | null {
...
const mgmtServer = createServer(async (req, res) => {
	...
	if (req.method === 'POST' && req.url === '/close-room') {
		...
		try {
			...
			if (connections) {
				connections.getConnections().forEach((conn: any) => {
					conn.close()
				})
				// 从内存中卸载文档
