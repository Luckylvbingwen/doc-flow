...
v-model="visibleLocal" title="添加引用" width="760px" :confirm-loading="confirmLoading" confirm-text="确认引用"
    ...
    <div class="arm-toolbar">
      <el-input v-model="keyword" placeholder="搜索文件名..." clearable @keyup.enter="reload" @clear="reload" />
...
async function reload() {
  loading.value = true
  try {
    const res = await apiSearchReferences(props.groupId, { keyword: keyword.value || undefined, page: 1, pageSize: 100 })
    if (res.success) {
