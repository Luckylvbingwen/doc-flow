...
interface GroupRow { id: bigint; name: string; description: string | null }
interface DocRow { id: bigint; title: string; group_id: bigint; group_name: string; updated_at: Date }
...
export default defineEventHandler(async (event) => {
	...
	const [groups, documents] = await Promise.all([
		...
		prisma.$queryRaw<DocRow[]>`
			SELECT d.id, d.title, d.group_id, g.name AS group_name, d.updated_at
