/**
 * POST /api/documents/:id/versions/:vid/restore-to-draft
 * 将已发布版本的内容还原到草稿（draft_content），供继续编辑
 *
...
export default defineEventHandler(async (event) => {
	...
	const vidStr = getRouterParam(event, 'vid')
	if (!vidStr || !/^\d+$/.test(vidStr)) {
		return fail(event, 400, INVALID_PARAMS, '版本 ID 非法')
	}
	const docId = BigInt(idStr)
	const vidBig = BigInt(vidStr)
	...
	if (![1, 2].includes(doc.status)) {
		return fail(event, 409, DOCUMENT_STATUS_INVALID, '当前文档状态不允许还原版本到草稿')
	}
	...
	const version = await prisma.doc_document_versions.findFirst({
		where: { id: vidBig, document_id: docId, deleted_at: null },
		select: { storage_key: true },
	})
	if (!version) return fail(event, 404, INVALID_PARAMS, '版本不存在')

	// 4. 从对象存储读取版本内容
	const buffer = await storage.getObject(version.storage_key)
