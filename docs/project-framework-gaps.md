...
### 1-2 Git Hooks（husky + lint-staged） ✅ 已完成
...
- **现状**：~~仅全局 `error.vue`~~ 已在 `prototype` 和 `default` layout 的 `<slot />` 外包裹 `NuxtErrorBoundary`
- **影响**：单个组件崩溃不再导致整页白屏，显示 fallback UI + 重试按钮
- **方案**：关键 layout 包裹 `NuxtErrorBoundary`，提供 `el-result` 降级界面
- **涉及**：`layouts/prototype.vue`、`layouts/default.vue`

### 2-3 XSS 清洗 ✅

- **现状**：~~diff 渲染等处直接 `v-html`，无过滤~~ 已引入 `dompurify`，创建 `useSanitize()` composable
- **影响**：所有 `v-html` 绑定均经过 `sanitize()` 过滤
- **方案**：`composables/useSanitize.ts` 封装 DOMPurify，替换所有 8 处 `v-html` 调用
- **涉及**：`DocPreview.vue`、`VersionCompareViewer.vue`、`CaptchaDialog.vue`、`pages/docs/file/[id].vue`

### 2-4 SEO 基础 ✅

- **现状**：~~零 `useHead` / `useSeoMeta` 调用~~ 已为全部 12 个页面添加 `useHead({ title })`
- **影响**：浏览器标签页显示页面名称而非 URL
- **方案**：每个页面 `useHead` 设置基础 title（格式：`页面名 - DocFlow`）
- **涉及**：所有 pages（12 个文件）

### 2-5 健康检查深度 ✅

- **现状**：~~`/api/health` 仅返回时间戳，不探测 DB~~ 已改造为探测 DB + Redis
- **影响**：K8s readiness probe 可反映数据库和 Redis 连接状态
- **方案**：每个依赖独立探测，返回 status + latency，整体状态 healthy / degraded
- **涉及**：`server/api/health.get.ts`

### 2-6 请求重试与离线兜底 ✅

- **现状**：~~`useAuthFetch` 无 retry，弱网直接失败~~ 已支持指数退避重试 + 离线检测
- **影响**：网络抖动 / 5xx 自动重试（最多 2 次），离线时 toast 提示
- **方案**：`useAuthFetch` 新增 `maxRetries` 参数，仅对网络错误 / 5xx 重试，4xx 不重试
- **涉及**：`composables/useAuthFetch.ts`

### 2-7 Bundle 分析 ✅

- **现状**：~~无 analyzer 配置~~ 已配置 `rollup-plugin-visualizer`
- **影响**：`ANALYZE=true nuxt build` 生成交互式 treemap 报告（含 gzip/brotli 体积）
- **方案**：`nuxt.config.ts` vite plugins 条件注入，平时零开销
- **涉及**：`nuxt.config.ts`、`.gitignore`

### 2-8 独立 404 页 ✅

- **现状**：~~靠 `error.vue` 兜底~~ 已新增 `pages/[...slug].vue` catch-all 路由
- **影响**：访问不存在的路径展示独立 404 界面，显示访问路径 + 导航按钮
- **方案**：`pages/[...slug].vue` catch-all 路由
- **涉及**：`pages/[...slug].vue`

### 2-9 暗黑模式 FOUC 修复 ✅

- **现状**：~~SSR 渲染时可能出现主题闪烁~~ 已在 `<head>` 注入 inline script 提前设置 `.dark`
- **影响**：首屏不再出现亮→暗闪烁
- **方案**：`useHead` 注入 inline script，读取 localStorage 在 HTML 解析阶段立即设置 `dark` 类名
- **涉及**：`app.vue`

### 2-10 缓存策略 ✅

- **现状**：~~静态资源无长缓存头，接口无服务端缓存~~ 已配置
- **影响**：`/_nuxt/**` 设为 immutable 长缓存，`/api/health` 短缓存 10s
- **方案**：`nuxt.config.ts` routeRules 配置 Cache-Control + Nitro cache
- **涉及**：`nuxt.config.ts`
...
| 编号 | 优先级 | 项目 | 技术选型 | 状态 |
|------|--------|------|----------|------|
| 基础 | P0 | Redis 基础设施 | `ioredis` + Docker Redis 7 | ✅ 已完成 |
| 0-1 | P0 | Rate Limiting | `rate-limiter-flexible` + Redis | ✅ 已完成 |
| 0-2 | P0 | 安全响应头 | `nuxt-security` | ✅ 已完成 |
| 0-3 | P0 | CORS 收紧 | `nuxt-security` corsHandler | ✅ 已完成 |
| 0-4 | P0 | Token 吊销机制 | 双令牌 + Redis blocklist | ✅ 已完成 |
| 0-5 | P0 | 密码安全体系 | `bcryptjs` | ✅ 已完成 |
| 0-6 | P0 | 结构化日志 | `pino` + `pino-pretty` | ✅ 已完成 |
| 1-1 | P1 | CI/CD 流水线 | GitHub Actions | ⬜ 待做 |
| 1-2 | P1 | Git Hooks | `husky` + `lint-staged` | ✅ 已完成 |
| 1-3 | P1 | Prisma Schema 对齐 | `prisma db pull` + 补全 | ⬜ 待做 |
| 1-4 | P1 | 测试补齐 | Vitest + Playwright | ⬜ 待做 |
| 1-5 | P1 | 统一错误码枚举 | `server/constants/error-codes.ts` | ✅ 已完成 |
| 2-1 | P2 | i18n 接线 | — | ⬜ 待做 |
| 2-2 | P2 | 组件级错误边界 | `NuxtErrorBoundary` | ✅ 已完成 |
| 2-3 | P2 | XSS 清洗 | `DOMPurify` | ✅ 已完成 |
| 2-4 | P2 | SEO 基础 | `useHead` / `useSeoMeta` | ✅ 已完成 |
| 2-5 | P2 | 健康检查深度 | DB + Redis ping | ✅ 已完成 |
| 2-6 | P2 | 请求重试与离线兜底 | 指数退避 retry | ✅ 已完成 |
| 2-7 | P2 | Bundle 分析 | `rollup-plugin-visualizer` | ✅ 已完成 |
| 2-8 | P2 | 独立 404 页 | `pages/[...slug].vue` | ✅ 已完成 |
| 2-9 | P2 | 暗黑模式 FOUC 修复 | `useHead` inline script | ✅ 已完成 |
| 2-10 | P2 | 缓存策略 | Redis + `defineCachedEventHandler` | ✅ 已完成 |
