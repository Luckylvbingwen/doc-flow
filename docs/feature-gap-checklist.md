...
|------|------|----------|----------|----------|--------|
| G-T1 | 飞书同步人员删除保护 | 飞书同步的成员不可手动删除 | ⚠️ 偏差 | 删除接口只检查 `immutable_flag`，未按 `source_type=2` 阻止删除飞书同步成员 | P2 |
...
|------|------|--------|----------|----------|----------|--------|
| G-F1 | 文件操作 | 跨组批量移动 | 单文件或批量多选移动到目标组，含权限处理 | ⚠️ 部分完成 | 单文件跨组移动已实现，**批量多选移动未实现** | P2 |
| G-F2 | 操作日志 | 按角色分范围查看 | 自动记录所有关键操作，按角色分范围查看 | ⚠️ 部分完成 | 日志记录已完整，但查看接口只做 `log:read` 权限门控，**未按角色限制可见范围**（如 dept_head 只看本部门日志） | P2 |
...
| G-F6 | 权限矩阵 | 上传下载角色在线编辑限制 | 上传下载(role=3)仅可编辑自己上传的文件 | ⚠️ 偏差 | 当前 `canEdit` 仅判断 `isOwner`（归属人），未按 PRD §4.3 对 role=3 限制为"仅自己上传的文件" | P2 |
| G-F7 | 权限矩阵 | 上传下载角色分享限制 | 上传下载(role=3)分享仅可选"可阅读" | ⚠️ 偏差 | 当前分享接口未按组内角色过滤可分享的权限级别，role=3 用户也能分享"可编辑" | P2 |
| G-F8 | 状态流转 | 邀请协同编辑入口 | 个人中心编辑中状态可邀请他人进入协同房间 | ⚠️ 部分完成 | 协同编辑本身已实现，但缺少编辑器内显式的"邀请协同"入口（发通知/链接邀人进入编辑房间） | P3 |
| G-F9 | 产品流程 | 组负责人交接-审批链替换 | 交接时将旧负责人在审批链中的节点自动替换为新负责人 | ❌ 缺失 | transfer-leader.post.ts 仅更新 owner+角色+通知，未同步替换审批模板节点中的旧负责人 | P2 |
| G-F10 | 共享文档 | 非组成员可见范围限制 | 非组成员只能看到组名列表，看不到组内文档 | ❌ 缺失 | tree.get.ts 返回所有组的文件节点给所有用户；GET /api/documents 未校验组成员身份（search.get.ts 已正确过滤） | P2 |
| G-F11 | 共享文档 | 系统管理员继承显示 | 系统管理员自动继承到公司层下所有子组，在成员列表中显示且不可删除/降权 | ⚠️ 偏差 | 功能通过 super_admin bypass 等效实现，但不在成员列表中显示为继承成员（immutable_flag=1） | P3 |
| G-F12 | 按部门 | 部门负责人换人时继承成员同步 | 飞书同步部门主管变更时，所有子组中旧负责人的继承成员记录应替换为新负责人 | ❌ 缺失 | `syncInheritedMembers()` 函数已定义但从未被调用，飞书同步只更新 owner_user_id 不同步子组成员 | P2 |
| G-F13 | 按部门 | 移除部门管理员阻塞校验 | 若管理员仍是某子组的组负责人，应阻塞移除并提示先移交 | ❌ 缺失 | `admins/[userId].delete.ts` 未检查目标用户是否仍为子组 owner | P2 |
| G-F14 | 按部门 | 部门管理员可见范围 | 部门管理员只能看到自己创建的子组（部门负责人可见全部） | ❌ 缺失 | DeptGroupListTab 和后端 API 返回该部门下所有组，未按创建者过滤 | P3 |
| G-F15 | 按产品线 | 产品线编辑/删除权限过紧 | PRD 允许产品线负责人编辑基本信息和删除产品线 | ⚠️ 偏差 | PUT/DELETE /api/product-lines/:id 后端仅允许 super_admin，前端 canEdit 含 pl_head 但请求会 403 | P2 |
| G-F16 | 按产品线 | 产品线负责人变更功能 | 支持变更产品线负责人，并同步替换所有子孙组中的继承成员 | ❌ 缺失 | 无变更 owner_user_id 的接口；PUT 只更新 name/description。负责人换人后子组继承成员无法同步 | P2 |
| G-F17 | 按产品线 | 管理员移除阻塞校验 | 若管理员仍是某子组的组负责人，应阻塞移除 | ❌ 缺失 | 与部门侧 G-F13 对称，admins/[userId].delete.ts 未检查 | P2 |
| G-F18 | 创建组 | 组名称/描述校验规则偏差 | PRD: 名称≤50字符+禁止特殊字符(/\:?"<>\)；描述≤200字符 | ⚠️ 偏差 | 实际 schema name max=150 无特殊字符正则校验；description max=500。前端 maxlength 也与 PRD 不一致 | P3 |
| G-F19 | 全局搜索 | 搜索结果缺少 PRD 要求的细节 | PRD: 组显示所属层级badge、文档显示版本号、结果显示计数、顶部"仅显示有权限的内容"提示 | ⚠️ 偏差 | 组结果无 scope 层级标签；文档结果无 version_no；无结果计数标题；无权限提示文字 | P3 |
| G-F20 | 文件列表 | 收藏按钮非独立列 | PRD 要求行尾独立星形按钮切换收藏 | ⚠️ 偏差 | 收藏功能仅在 ··· 下拉菜单中，未作为独立列按钮展示 | P3 |
| G-F21 | 文件列表 | 审批提示条缺跳转链接 | PRD 要求"前往审批中心→"链接(reviewing>0 时显示) | ⚠️ 偏差 | 当前仅纯文字提示，无跳转链接 | P3 |
| G-F22 | 文件列表 | 子组卡片缺成员数 | PRD 要求卡片显示成员数 | ⚠️ 偏差 | 卡片 footer 无 memberCount 字段 | P3 |
| G-F23 | 文件列表 | 编辑副本确认弹窗 | PRD 要求结构化弹窗(目标文件+版本+归属人+黄色警告) | ⚠️ 偏差 | 当前直接调 API 后 msgConfirm，无完整确认 Modal UI | P3 |
...
| G-F28 | 文件详情 | 页内版本对比缺同步滚动 | PRD 要求左右并排同步滚动 | ⚠️ 偏差 | 全屏对比有 syncScroll()，页内 inline 对比模式无同步滚动逻辑 | P3 |
...
| G-F31 | 在线编辑器 | 下载按钮缺失 | PRD 要求顶栏有下载按钮，toast"已下载"并触发文件下载 | ❌ 缺失 | 编辑器顶栏无下载功能 | P3 |
| G-F32 | 在线编辑器 | 删除/取消编辑入口缺失 | PRD 要求：新建/草稿→confirm 后软删除到回收站；编辑副本→confirm 提示"取消编辑将不保存修改" | ❌ 缺失 | 顶栏无删除/取消编辑按钮和对应 confirm 弹窗逻辑 | P2 |
| G-F33 | 在线编辑器 | 标题校验规则偏差 | PRD 要求 maxlength=100 且禁止 `/\:*?"<>\|` 特殊字符 | ⚠️ 偏差 | 当前 maxlength=255，无特殊字符校验 | P3 |
| G-F34 | 在线编辑器 | 提交发布 confirm 文案缺失 | PRD 要求先弹出 confirm："提交后将进入审批流程，审批期间文档不可编辑..." | ⚠️ 偏差 | 当前直接打开 PublishModal，缺少前置 confirm 对话框 | P3 |
| G-F35 | 在线编辑器 | hocuspocus 缺文档级权限校验 | PRD 要求上传下载角色不能加入协同房间 | ⚠️ 偏差 | hocuspocus onAuthenticate 仅验证 JWT 有效性，未校验用户对该文档的编辑权限(任何登录用户可连接任意房间) | P1 |
| G-F36 | 在线编辑器 | 返回路径未区分来源 | PRD 要求来自共享文档→返回 repos；来自个人中心→返回 personal | ⚠️ 偏差 | 统一使用 router.back()，未根据来源区分跳转目标 | P3 |
| G-F37 | 历史快照 | Ctrl+S 手动保存缺失 | PRD 要求 Ctrl+S 触发手动保存（type=2），创建快照记录 | ❌ 缺失 | 无键盘事件监听，DB schema 预留 type=2 但从未使用 | P2 |
| G-F38 | 历史快照 | 当前活跃蓝色圆点缺失 | PRD 要求时间线上当前活跃节点使用蓝色实心圆点区分 | ❌ 缺失 | 无 active 状态标识逻辑，所有快照节点均为灰色 | P3 |
| G-F39 | 历史快照 | 版本节点不支持预览 | PRD 要求版本节点也可预览（在编辑区临时只读展示） | ❌ 缺失 | 点击预览时直接 msgError 拒绝，提示"版本预览暂不支持" | P3 |
| G-F40 | 历史快照 | 共享文档应隐藏快照 | PRD 要求共享文档仅展示版本节点，隐藏编辑快照 | ❌ 缺失 | 无前后端区分逻辑，SnapshotDrawer 始终返回全部快照+版本 | P2 |
| G-F41 | 历史快照 | 命名快照入口偏差 | PRD 要求在顶栏"..."菜单内直接创建快照 | ⚠️ 偏差 | 入口放在 SnapshotDrawer 内部的"+ 保存快照"按钮，需先打开历史抽屉 | P3 |
| G-F42 | 历史快照 | 返回时未创建快照记录 | PRD 要求返回上级页面时触发保存（与自动保存同类型） | ⚠️ 偏差 | handleBack 仅 flushSave 写入 draft_content，未创建 doc_document_snapshots 记录 | P3 |
| G-F43 | 协同编辑 | 房间无生命周期管理 | PRD 要求文档发布/删除时销毁房间 | ❌ 缺失 | hocuspocus 无 onStoreDocument/afterUnloadDocument 钩子，发布/删除时无通知关闭房间的代码 | P1 |
| G-F44 | 协同编辑 | 批注未走实时同步通道 | PRD 要求批注新增/回复/解决实时同步（独立于正文） | ❌ 缺失 | 批注完全走 HTTP REST API，多人同时批注看不到对方实时新增 | P2 |
| G-F45 | 协同编辑 | Presence 心跳未配置 5s | PRD 要求 5s 心跳超时即标记离线 | ⚠️ 偏差 | hocuspocus 未设置 timeout 选项，y-websocket 默认行为（30s），不满足 5s 要求 | P2 |
| G-F46 | 协同编辑 | 光标位置同步缺 100ms 节流 | PRD 要求光标位置 100ms 节流同步 | ⚠️ 偏差 | 依赖 milkdown-collab 默认行为，未显式配置 100ms 节流，且 Awareness 未设置 cursor/selection 数据 | P3 |
| G-F47 | 协同编辑 | 飞书邀请协作未实现 | PRD 要求创建者可在飞书中邀请协作者加入房间 | ❌ 缺失 | 全局无飞书邀请协作相关代码 | P2 |
| G-F48 | 协同编辑 | 发布后组成员权限自动分配 | PRD 要求草稿发布后组内成员自动获得"可编辑"权限 | ❌ 缺失 | 发布流程无自动为组成员创建 doc_document_permissions 的逻辑 | P2 |
| G-F49 | 链接分享 | 共享文档列表行右键"复制分享链接"缺失 | PRD 要求共享文档列表行右键菜单含"复制分享链接"入口 | ❌ 缺失 | GroupFilesPanel 无 contextmenu 事件和分享菜单项 | P3 |
| G-F50 | 链接分享 | 分享权限约束基于全局 RBAC 而非文档级 | PRD 要求分享权限不能超过分享人**对该文档**的权限级别 | ⚠️ 偏差 | 后端用全局 `doc:edit` 权限码判断，非文档级粒度；若用户有全局编辑角色但对某文档仅有可阅读，仍可分享可编辑链接 | P2 |
| G-F51 | 链接分享 | 编辑器/个人中心入口未传 canEdit prop | PRD 要求可阅读用户不可选"可编辑"选项 | ⚠️ 偏差 | editor/[id].vue 和 profile.vue 调用 ShareLinkModal 时未传 canEdit prop，UI 上可阅读用户仍看到可编辑选项（后端拦截不受影响） | P3 |
| G-F52 | 选字批注 | 选字长度限制边界偏差 | PRD 要求 1~500 字符 | ⚠️ 偏差 | 条件 `<=1 \|\| >=500` 导致实际为 2~499，应改为 `<1 \|\| >500` | P3 |
| G-F53 | 选字批注 | 编辑副本提示未解决批注时机不符 | PRD 要求在创建确认弹窗中追加提示行（事前） | ⚠️ 偏差 | 当前实现是先调 API 创建副本成功后才弹窗提示，非事前在确认弹窗中追加 | P3 |
| G-F54 | 选字批注 | 冻结卡片灰度样式待补 | PRD 要求冻结批注卡片整体灰度 60% | ⚠️ 偏差 | 有 `.annotation-item--frozen` class 但 SCSS 中缺少 `opacity: 0.6` 或 `filter: grayscale` 样式定义 | P3 |
| G-F55 | 归属人转移 | 飞书卡片缺组名和版本信息 | PRD 卡片要求含"文档所在组: {组名}/{部门or产品线}"+"当前版本: v{X.Y}" | ⚠️ 偏差 | M10 通知模板仅传发起人+文件名，未传组名和版本号 | P3 |
| G-F56 | 归属人转移 | 飞书卡片不会就地更新 | PRD 要求同意后卡片更新为"✓已同意"，拒绝后更新为"✗已拒绝" | ❌ 缺失 | 当前只发新通知，不更新原卡片（需飞书 update API + 存储 message_id） | P3 |
| G-F57 | 归属人转移 | 同意后未通知新归属人自己 | PRD 要求"消息通知双方" | ⚠️ 偏差 | 仅通知了原归属人(M11)，未发给新归属人确认成功消息 | P3 |
...
| G-F59 | 审批中心 | 全屏预览器缺"上传新版本"入口 | PRD 要求普通模式版本侧栏有"上传新版本"按钮 | ❌ 缺失 | FullscreenPreviewer 版本侧栏无上传入口，两种模式都没有 | P3 |
| G-F60 | 审批中心 | 驳回 toast 文案未对齐 PRD | PRD 要求"已驳回，驳回原因已通知提交人" | ⚠️ 偏差 | 当前前端 fallback 和服务端返回均仅为"已驳回"，缺少后半句 | P3 |
| G-F61 | 审批中心 | 审批链当前节点样式偏差 | PRD 要求当前节点显示蓝色"..."省略号 | ⚠️ 偏差 | 当前是蓝色高亮+脉冲动画显示头像首字母，无省略号 glyph | P3 |
| G-F62 | 个人中心 | 文件名不可点击跳转 | PRD 要求文件名为链接(草稿/编辑中→编辑器；已发布/审批中→详情页) | ❌ 缺失 | 标题 slot 为普通 span 无点击绑定，跳转仅通过操作按钮触发 | P2 |
| G-F63 | 个人中心 | 收藏黄星缺失 | PRD 要求已收藏文档在文件名旁显示黄色星标 | ❌ 缺失 | 标题区无 Star 图标，PersonalDocItem 类型也无 isFavorited 字段 | P3 |
| G-F64 | 个人中心 | 搜索非实时过滤 | PRD 要求输入框实时过滤（防抖） | ⚠️ 偏差 | 当前仅在回车或 clear 时触发 onFilterChange，无 @input/watch 防抖 | P3 |
| G-F65 | 个人中心 | 子 Tab 来源列未隐藏 | PRD 要求子 Tab（我创建的/分享给我的/个人收藏）不显示来源列 | ⚠️ 偏差 | 所有 Tab 固定显示"来源"列，子 Tab 渲染为"—" | P3 |
| G-F66 | 个人中心 | 编辑中+分享可编辑缺提交发布 | PRD 操作矩阵要求编辑中+分享给我的(可编辑)有"提交发布"菜单项 | ⚠️ 偏差 | 当前 personal-matrix 仅 owner 才暴露 publish，共享编辑者看不到 | P2 |
| G-F67 | 个人中心 | 已发布编辑缺前置确认弹窗 | PRD 要求已发布(我创建的)点编辑时先弹"编辑副本确认"弹窗 | ⚠️ 偏差 | 当前直接调 apiCreateEditCopy，仅冲突时弹"加入协同"确认（与 G-F23 关联） | P3 |
| G-F68 | 个人中心 | 离职移交 TAB 缺 NEW 标记 | PRD 要求"离职移交"TAB 旁显示 NEW badge | ❌ 缺失 | TabBar 组件只渲染标签和计数，无额外 badge 能力 | P3 |
| G-F69 | 个人中心 | 离职移交展开后非表格视图 | PRD 要求展开后为文件表格(列: 文件名/版本/状态/原所属组/交接日期/操作) | ⚠️ 偏差 | HandoverAccordion 展开后为卡片列表(标题+摘要+状态badge+查看按钮)，缺交接日期列和更多操作(下载/删除) | P2 |
| G-F70 | 回收站 | 预览接口条件冲突可能导致"查看"不可用 | PRD 要求查看仅展示改版正文 | ⚠️ 偏差 | preview.get 筛选 `deleted_at IS NOT NULL` 而列表用 `deleted_at IS NULL && status=6`，两者条件冲突可能导致预览查不到文件 | P2 |
| G-F71 | 回收站 | 缺少"已删除文件 X 个文件"计数文案 | PRD 原型顶部显示"已删除文件 N 个文件" | ⚠️ 偏差 | 当前标题副文案为固定"已删除的文件在此保留 30 天"，无动态总数展示 | P3 |
| G-F72 | 回收站 | 未按组分组展示 | PRD 要求列表形式按组分组展示已删除文件 | ⚠️ 偏差 | 当前为普通表格平铺+原组列+组筛选，非"按组分组块"展示 | P3 |
| G-F73 | 操作日志 | 文件编辑类日志未落库 | PRD 定义"文件编辑"含创建编辑副本/在线编辑保存(自动/Ctrl+S/快照) | ❌ 缺失 | DOC_DRAFT_CREATE/DOC_EDIT_SAVE/DOC_SNAPSHOT_CREATE 仅在常量映射中定义，无业务调用 writeLog | P2 |
| G-F74 | 操作日志 | 多个日志子项未实际埋点 | PRD 要求完整覆盖所有操作子项 | ⚠️ 偏差 | 飞书导入上传/文件移动过期/申请编辑权限/飞书同步成员/离职交接/批注相关(添加/回复/解决)均仅有常量定义无业务写入 | P2 |
| G-F75 | 操作日志 | 按角色分范围查看未实现 | PRD 要求不同角色看到不同范围的日志(已记录于G-F2) | ⚠️ 偏差 | 当前仅 requirePermission(log:read) 门控，无按部门/产品线/角色范围裁剪 where 条件 | P2 |
...
| G-F78 | 通知中心 | 飞书卡片内容规范不符 | PRD 要求结构化正文(操作人+所属组+操作时间+操作描述) | ⚠️ 偏差 | 当前只有分类头+title/content，未呈现 PRD 要求的完整结构化卡片正文 | P3 |
| G-F79 | 通知中心 | M13 跨组移动过期分支未落地 | PRD 要求移动请求超时自动过期并通知发起人 | ❌ 缺失 | 无跨组移动过期扫描任务，仅有同意/拒绝分支 | P2 |
| G-F80 | 通知中心 | M17 触发语义反转 | PRD 要求"有人分享给我→通知被分享人"，当前是"访问链接者触发通知给分享者" | ⚠️ 偏差 | 通知发送时机和收件人与 PRD 定义相反 | P2 |
| G-F81 | 通知中心 | 站内通知卡片缺操作按钮 | PRD 要求通知卡片按类型显示"同意/拒绝/查看文件/查看组"等操作按钮 | ⚠️ 偏差 | NotificationCard 仅有统一点击行为，无按 M 类型展示差异化动作按钮 | P2 |
| G-F82 | 系统管理 | 部门负责人行"角色管理"按钮未禁用 | PRD 要求部门负责人行为只读(飞书同步) | ⚠️ 偏差 | 前端"角色管理"按钮对所有活跃非super_admin行开放，未针对 dept_head 行禁用或显示"只读"文案 | P3 |
| G-F83 | 文档引用 | 添加引用弹窗缺组筛选下拉 | PRD 要求有"所有可见组"下拉(排除当前组，显示"组名(N个文件)") | ❌ 缺失 | 弹窗仅有搜索框+计数，无 el-select 组筛选(前端 API 已预留 sourceGroupId 但未使用) | P2 |
| G-F84 | 文档引用 | 引用行缺浅蓝背景 | PRD 要求引用文档行使用 #fafcff 浅蓝背景 | ❌ 缺失 | DataTable 支持 rowClassName 但 GroupFilesPanel 未传入引用行样式 | P3 |
| G-F85 | 文档引用 | 引用图标颜色非主色调 | PRD 要求 SVG 链接图标使用主色调 | ⚠️ 偏差 | 当前引用图标颜色为绿色 #10b981，非主色 --df-primary(#2563eb) | P3 |
| G-F86 | 文档引用 | 引用行操作菜单未限管理员 | PRD 要求"..."更多(含取消引用)仅组管理员可见 | ⚠️ 偏差 | row.isReference 分支直接展示 unreference，未加管理员条件判断 | P2 |
| G-F87 | 文档引用 | 引用行仍可能出现非法操作 | PRD 要求引用行仅含"详情"+"取消引用" | ⚠️ 偏差 | 引用行更多菜单可能出现编辑等操作(判定在引用分支之前)，且引用行不支持收藏/置顶入口 | P2 |
| G-F88 | 文档引用 | 引用文档详情页缺差异化处理 | PRD 要求引用详情页有只读提示条/标题链接图标/仅下载按钮/返回目标组 | ❌ 缺失 | 详情页无引用模式识别，展示全部操作按钮，返回逻辑按源组而非引用目标组 | P1 |
| G-F89 | 文档引用 | 取消引用确认框不符 PRD | PRD 要求含文档信息卡片(灰色背景+来源)+补充说明文字 | ⚠️ 偏差 | 当前仅简短文本确认，无卡片式 UI | P3 |
| G-F90 | 文档引用 | 批量移除未触发引用自动失效 | PRD 要求源文档被移除时自动删除引用+通知 | ❌ 缺失 | batch-remove.post.ts 把文档改为 status=1 但未调用 cleanupDocumentReferences | P1 |
| G-F91 | 文档引用 | 翻页器缺"含 N 条引用"标注 | PRD 要求分页器标注"含 N 条引用" | ❌ 缺失 | 后端已返回 referenceCount 但前端未消费，Pagination 未注入提示文案 | P3 |
| G-F92 | 文档引用 | 搜索接口缺可见组权限过滤 | PRD 要求仅展示管理员有权限查看的组的文档 | ⚠️ 偏差 | search.get 查询全局已发布文档(排除当前组)，未加入可见组/权限范围过滤 | P2 |
| G-F93 | 文档引用 | 引用置顶上下文不一致 | PRD 要求引用文档置顶在引用组生效(独立于源组) | ⚠️ 偏差 | 详情接口/pin接口按源组 group_id 读写，不支持引用组独立置顶 | P2 |
...
| G-F96 | 非功能-安全 | 操作日志缺 DB 层面不可篡改保障 | PRD 要求"操作日志不可篡改，仅 INSERT" | ⚠️ 偏差 | 应用层仅 INSERT/SELECT，但未在数据库层面 REVOKE DELETE/UPDATE 或加触发器保护 | P2 |
| G-F97 | 非功能-性能 | 未见并发用户上限配置 | PRD 要求"支持 50 人同时在线" | ⚠️ 偏差 | WebSocket 管理用内存 Map 无连接上限；hocuspocus 未配置 maxConnections；缺负载测试基线 | P3 |
| G-F98 | 非功能-视觉 | 图标风格未完全统一为线性 | PRD 要求"统一线性图标风格" | ⚠️ 偏差 | 主流使用 Element Plus 线性图标，但部分场景混用 Filled 图标(如 WarningFilled、SuccessFilled) | P3 |
