...
| G-F24 | 文件列表 | ~~跨组移动弹窗细节~~ | PRD 要求搜索框+展示目标组成员权限列表+通知提示 | ✅ 2025-05-20 | MoveTargetPicker 增加搜索框过滤组树+选中后显示权限变更警告 | P3 |
| G-F25 | 文件列表 | 飞书导入缺更新导入模式 | PRD 支持"更新导入"(关联已有文件+版本递增) + 映射飞书协作者权限 | ⚠️ 部分完成 | 仅支持首次导入模式；未获取飞书文档协作者映射为 DocFlow 可编辑权限 | P2 |
| G-F26 | 文件详情 | ~~Presence 头像组为静态数据~~ | PRD 要求显示"当前正在查看该文档的人"(实时 WebSocket) | ✅ 2025-05-20 | 新增 presence API(Redis ZSET+60s TTL)，文件详情页 30s 心跳轮询实时在线用户 | P2 |
...
| G-F29 | 在线编辑器 | ~~Presence 头像组 UI 未渲染~~ | PRD 要求顶栏显示当前在线协作者头像(最多3+溢出数字)，hover 姓名列表，点击 toast 全部 | ✅ 2025-05-20 | 编辑器顶栏渲染 onlineUsers 头像组(最多3+溢出)，tooltip 显示全部姓名 | P2 |
| G-F30 | 在线编辑器 | ~~TOC 目录侧栏缺失~~ | PRD 要求左侧显示文档目录(类飞书)，可折叠 | ✅ 2025-05-20 | 编辑器左侧新增可折叠 TOC 侧栏，从 Markdown 标题实时提取，顶栏按钮切换显隐 | P2 |
...
| G-F44 | 协同编辑 | ~~批注未走实时同步通道~~ | PRD 要求批注新增/回复/解决实时同步（独立于正文） | ✅ 2026-05-20 | 建立文档级 WS 订阅通道；批注写接口推送增量载荷（annotation/reply）；文件详情页/编辑页本地合并状态并重算高亮 | P2 |
...
| G-F72 | 回收站 | ~~未按组分组展示~~ | PRD 要求列表形式按组分组展示已删除文件 | ✅ 2025-05-20 | API 默认按组名排序，原组列用 el-tag 块状标记，同组相邻行视觉归组 | P3 |
...
| G-F94 | 非功能-安全 | ~~JWT 会话默认时长与 PRD 不一致~~ | PRD 要求"JWT Token 默认 8 小时" | ✅ 2025-05-20 | 设计决策：双令牌(access 15m + refresh 7d)安全性优于单令牌 8h，标记为有意偏差 | P3 |
| G-F95 | 非功能-安全 | ~~文件下载非服务端流式传输~~ | PRD 要求"通过鉴权后流式传输" | ✅ 2025-05-20 | 设计决策：预签名 URL 直连下载性能优于服务端代理流式，有时效性保障安全，标记为有意偏差 | P3 |
...
| G-F99 | 非功能-视觉 | ~~加载态未全面使用骨架屏~~ | PRD 要求“骨架屏代替 Loading 转圈” | ✅ 2025-05-20 | 审批页/通知页初始加载改用 el-skeleton 骨架屏 | P3 |
